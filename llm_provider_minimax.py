...
class MiniMaxProvider:
    ...
    def __init__(
        ...
        **kwargs
        ...
        Args:
            model: 模型名称
            api_key: API密钥
            base_url: API地址（默认使用MiniMax API）
            temperature: 温度参数
            max_tokens: 最大token数
        """
        self.model = model
        ...
        # 初始化 ChatOpenAI（MiniMax兼容 OpenAI API）
    ...
    def chat_with_tools(
        ...
        # 转换 tools 格式（需要 type: "function" 外层）
        minimax_tools = []
        for tool in tools:
            if "type" not in tool:
                minimax_tools.append({
                    "type": "function",
                    "function": {
                        "name": tool.get("name"),
                ...
                minimax_tools.append(tool)

        # 构建请求体
        tool_choice_config = None
        if tool_choice == "required" and minimax_tools:
            tool_choice_config = {
                "type": "function",
                "function": {"name": minimax_tools[0]["function"]["name"]}
            }
        ...
        print(f"  调用MiniMax API (Function Calling)...")
